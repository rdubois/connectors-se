...
class CsvBlobReaderTest extends AdlsGen2TestBase {
    ...
    @Test
    void readRecord() {
        this.inputConfiguration.getDataSet().setFormat(FileFormat.CSV);
        this.dataSet.getCsvConfiguration().getCsvFormatOptions().setFieldDelimiter(CSVFieldDelimiter.COMMA);
        this.dataSet.getCsvConfiguration().getCsvFormatOptions().setRecordDelimiter(CSVRecordDelimiter.LF);
        ...
        ClientGen2Fake fake = new ClientGen2Fake(new FakeResponse<>(200, paths, null, null)) {
            ...
            public Response<InputStream> pathRead(Map<String, String> headers, String filesystem, String path, Integer timeout,
                ...
                if ("/paht1/file1.txt".equals(path)) {
                    input = new ByteArrayInputStream("Fic1,line1\nFic1,line2".getBytes(StandardCharsets.UTF_8));
                } else if ("/paht1/file2.txt".equals(path)) {
                    input = new ByteArrayInputStream("Fic2,line1\nFic2,line2".getBytes(StandardCharsets.UTF_8));
                } else {
        ...
        final CsvBlobReader reader = new CsvBlobReader(this.inputConfiguration, this.recordBuilderFactory, gen2Service,
        ...
        Assertions.assertEquals("Fic2", record4.getString("field1"));
        Assertions.assertEquals("line2", record4.getString("field2"));
